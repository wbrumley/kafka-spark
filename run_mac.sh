#!/bin/bash
# Mac-specific script for Kafka-Spark Security Analytics System
# This script is designed for macOS with Docker Desktop

set -e  # Exit on any error

echo "======================================================"
echo "  Kafka-Spark Security Analytics - Mac Launcher"
echo "======================================================"
echo ""
...
check_docker() {
    ...
    if ! docker info &> /dev/null; then
        echo "Error: Docker daemon is not running. Please start Docker Desktop for Mac."
        exit 1
    fi
    
    echo "Docker is running."
    
    # On Mac, use docker compose subcommand (comes with Docker Desktop)
    echo "Checking Docker Compose..."
    if docker compose version &> /dev/null; then
        echo "Docker Compose is available (via docker compose command)."
    else
        echo "Error: Docker Compose is not available. Please update Docker Desktop for Mac."
        exit 1
    fi
}
...
start_kafka() {
    ...
    # Start Kafka and Zookeeper using docker compose subcommand
